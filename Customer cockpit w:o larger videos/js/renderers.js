...
export const renderInteractiveOrgChart = (container, data, options = {}) => {
 ...
 export const renderSaasComparisonPage = (container, data, options = {}) => {
    function render() {
        ...
        const videoHtml = videoId ? `
            <div class="bg-white p-6 rounded-xl border border-slate-200 mb-6">
                <div class="flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-slate-800">${videoTitle}</h3>
                    <button 
                        data-action="open-video-modal" 
                        data-video-src="assets/videos/${videoId}"
                        class="inline-flex items-center gap-2 bg-shyftplan-purple text-white font-semibold py-2 px-4 rounded-lg hover:bg-violet-700 transition-colors">
        ...
        const analogyHtml = ''; // Analogy box was removed previously
        const toggleHtml = `
            <div class="flex justify-center items-center gap-3 my-8">
                <span class="font-medium text-slate-600">Problem</span>
                <button type="button" id="saas-mode-toggle" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-shyftplan-purple ${agileMode ? 'bg-shyftplan-purple' : 'bg-slate-300'}" role="switch" aria-checked="${agileMode}">
                    <span class="sr-only">Lösung anzeigen</span>
                    <span aria-hidden="true" class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${agileMode ? 'translate-x-5' : 'translate-x-0'}"></span>
                </button>
        ...
        const renderPoints = (points) => {
            ...
            return points.map(point => `<div class="flex items-start"><div class="h-1.5 w-1.5 bg-slate-700 rounded-full mr-2.5 mt-2 flex-shrink-0"></div><span class="text-sm text-slate-600">${point}</span></div>`).join('');
        };
        ...
        const comparisonHtml = `
            <div class="border border-slate-200 rounded-xl overflow-hidden">
                <!-- Header Row -->
                <div class="grid grid-cols-3 text-sm border-b border-slate-200">
                    <div class="p-4 bg-slate-50 font-semibold text-slate-700"></div>
                    <div class="p-4 bg-slate-50 font-semibold text-slate-700">${comparison.oldWorld.title}</div>
                    <div class="p-4 font-semibold text-white ${agileMode ? 'bg-green-600' : 'bg-red-600'}">${rightColData.title}</div>
                </div>

                <!-- Data & Result Rows -->
                ${comparison.categories.slice(0, -1).map((category, index) => `
                    <div class="grid grid-cols-3 text-sm border-b border-slate-200 last:border-b-0">
                        <div class="p-4 bg-slate-50 font-semibold text-slate-700">${category}</div>
                        <div class="p-4 space-y-2">${renderPoints(comparison.oldWorld.points[index])}</div>
                        <div class="p-4 space-y-2">${renderPoints(rightColData.points[index])}</div>
                    </div>
                `).join('')}

                <!-- Result Row -->
                <div class="grid grid-cols-3 text-sm">
                    <div class="p-4 bg-slate-50 font-semibold text-slate-700">${comparison.categories.slice(-1)[0]}</div>
                    <div class="p-4">
        ...
        container.innerHTML = (options.presentationMode ? '' : createHeader(data.title, data.subtitle)) + videoHtml + toggleHtml + comparisonHtml;
        lucide.createIcons();
    }
    render();
